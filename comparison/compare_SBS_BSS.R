...
for(epoch in 1:niter){
    ...
    ### apply the SBS method
    sbs_cps <- vector('list', M)
    for(i in 1:M){
        fit_sbs <- sbs.alg(t(subjects[[i]]), cp.type = 2, scales = -1)
        cp <- fit_sbs$ecp
        if(length(cp) != 0){
            sbs_cps[[i]] <- cp
        }
    ...
    sbs_cp_vec <- unlist(sbs_cps)
    cl <- fviz_nbclust(matrix(sbs_cp_vec, length(sbs_cp_vec), 1), FUN = kmeans, nstart = 25, 
    ...
    sbs_cl <- kmeans(sbs_cp_vec, cl.number)
    
    sbs_result <- cbind(sbs_cp_vec, sbs_cl$cluster)
    cluster_result[[epoch]] <- sbs_result
}

save(cluster_result, file = "final_cluster.RData")

###### analyze results
load("final_cluster.RData")
