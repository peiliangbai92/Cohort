...
M <- 10
r_T <- 10
tau_true <- c(floor(T/5), floor(2*T/5), floor(3*T/5), floor(4*T/5))
...
for(i in 1:M){
    tau_sub[[i]] <- c(tau_true[1] + sample(seq(-r_T, r_T, 1), 1), 
                      ...
                      tau_true[3] + sample(seq(-r_T, r_T, 1), 1), 
                      tau_true[4] + sample(seq(-r_T, r_T, 1), 1))
}
...
for(epoch in 1:nepoch){
    ...
    for(i in 1:M){
        ...
        try <- simu_var(method = "sparse", 
                        ...
                        signals = c(-entry, entry, -entry, entry, -entry))
