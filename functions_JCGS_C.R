...
soft <- function(L,weight,lambda){
  for (i in 1:length(L[1,])){
    ...
    else if ( L[i] < -lambda){L[i] <- L[i] + lambda}
    else{L[i] <- 0}
  }
  return(L)
}
...
soft.full <- function(L,lambda,k,p,n){
  ...
  for (i in 1:length(L[1,])){
    for(j in 1:length(L[,1])){
      ...
      else if ( L[j,i] < -lambda){L[j,i] <- L[j,i] + lambda}
      else if ( abs(L[j,i]) <= lambda){L[j,i] <- 0}
    }
  }
  return(L)
}
...
var.break.fit.block <- function(method,data, weight = NULL, lambda, lambda2, p, max.iteration = 1000, tol = 10^(-4), step.size = 10^(-3),initial.phi = NULL, blocks, cv.index){
  method.full <- c("LASSO");
  if ( !(method %in% method.full) ){print("ERROR"); break; }
  ...
  for(jjj in 1:n.new){
    #print(D.sum.new[((jjj-1)*k*p+1):(jjj*k*p),])
    min.eigen <- min(eigen(D.sum.new[((jjj-1)*k*p+1):(jjj*k*p),])$values);
    # print("EIGEN::::::::::::::::::::::::::::::");
    # print(min.eigen)
    add.pd <- 0;
    if(min.eigen <= 0){add.pd <- (10)*abs(min.eigen);}
    D.sum.new.inv[((jjj-1)*k*p+1):(jjj*k*p),] <- solve( D.sum.new[((jjj-1)*k*p+1):(jjj*k*p),] + add.pd*diag(k*p) );
    # D.sum.new.inv[((jjj-1)*k*p+1):(jjj*k*p),] <- solve( D.sum.new[((jjj-1)*k*p+1):(jjj*k*p),] + 1*(1*10^(-10))*diag(k*p) );
    }
  # D.sum.new.inv <- sapply(c(1:n), function(jjj)  solve(D.sum.new[((jjj-1)*k*p+1):(jjj*k*p),])   )
  ...
  if (method == 'LASSO'){
    for (ll in 1:length(lambda)){
      ...
      while( l < max.iteration){
        ...
        phi.temp.soft <- matrix(0,k,k*p*n.new);
        phi.temp.soft[,1:(k*p)] <- soft.full(phi.hat[,1:(k*p)],lambda2)
        temp.1 <- phi.hat[,1:(k*p)];
        for(z.1 in 2:n.new){
          temp.2 <- temp.1 + phi.hat[,( (z.1-1)*(k*p) + 1):(z.1*(k*p))];
          temp.1.soft <- soft.full(temp.1,lambda2);
          temp.2.soft <- soft.full(temp.2,lambda2);
          phi.temp.soft[,( (z.1-1)*(k*p) + 1):(z.1*(k*p))] <- temp.2.soft - temp.1.soft;
          temp.1 <- temp.2;
        }

        phi.new <- phi.temp.soft;
...
##################################################################################
break.var.local.new <- function(data, eta, p, max.iteration = 1000, tol = 10^(-4), step.size = 10^(-3), pts, an){
  k <- length(data[1,]); T <- length(data[,1]); m <- length(pts); 
  ...
  #print(bounds.1)
  # ptm.temp <- proc.time()
  # r.1 <- foreach(mm=1:(2*m), .inorder = FALSE, .export = c("sourceCpp")) %dopar% {
  #   sourceCpp("var_lasso_brk.cpp")
  #   temp <- vector("list",2);
  #   data.temp <- data[(bounds.1[[mm]][1]):(bounds.1[[mm]][2]),];
  #   #try <- var.lasso.brk(data = data.temp, weight = NULL, eta, p, max.iteration = 1000, tol = tol)
  #   #####use rcpp
  #   #print("use rcpp in step 2")
  #   try <- var_lasso_brk(data = data.temp, eta, p, 1000, tol = tol)
  #   temp[[1]] <- try$pred.error;
  #   temp[[2]] <- try$phi.hat;
  #   temp
  # }
  # print(proc.time() - ptm.temp)
  # print("Next!!")
  ...
  for(mm in 1:(2*m)){
    ...
    phi.local.1 <- c(phi.local.1, try$phi.hat)
    #print(mm)
  }
  #print("Next!!")
  #print(proc.time() - ptm.temp)
  ...
  for(mm in 1:m){
    ...
    phi.local.2 <- c(phi.local.2, try$phi.hat)
  }
  #print(L.n.2)
...
second.step.local.new <- function(data, eta, p, max.iteration = 1000, tol = 10^(-4), step.size = 10^(-3), pts, an){
  m <- length(pts); if( m == 0){break;}
  try <- break.var.local.new(data, eta, p, max.iteration = 1000, tol = tol, step.size = 10^(-3), pts, an);
  ...
  print("first step points:");print(pts)
  
  
  #####use kmeans to cluster the V (old version)
  #clus.1 <- kmeans(V, centers = 1); fit.1 <- clus.1$betweenss/(clus.1$totss+0.001); print(fit.1);
  #if(fit.1 > 0.8){omega <- max(V);}
  # if( fit.1 <= 0.8 && length(unique(V)) >2 ){
  #   #consider 2 clusters for fit.2
  #   print("consider 2 clusters for fit.2")
  #   clus.2 <- kmeans(V, centers = 2); fit.2 <- clus.2$betweenss/clus.2$totss; print(fit.2);
  #   loc <- clus.2$cluster;
  #   if( clus.2$centers[1] > clus.2$centers[2]  ){omega <- min(V[which(loc==1)]) -1 ;}
  #   if( clus.2$centers[1] < clus.2$centers[2]  ){omega <- min(V[which(loc==2)]) -1 ;}
  # }
  # if( fit.1 <= 0.8 && length(unique(V)) <= 2 ){
  #   #omega <- max(V);
  #   omega <- min(V) - 1;
  ...
  #####use kmeans to cluster the V (new version 05/09/2020)
  #add two bounary pts as redunant pts (by assumption, their V values shoule be extremly small)
  T <- length(data[,1]);
  pts.redundant <- c(an+p, T-an)
  try.redundant <- break.var.local.new(data, eta, p, max.iteration = 1000, tol = tol, step.size = 10^(-3), pts.redundant, an);
  ...
  print("redundant points:");print(pts.redundant)
  print("V redundant value:");print(V.redundant)
  V <- c(V,rep(V.redundant,length(V)))
  # omega <- 4500   ### for using all data
  omega <- 1500
  ...
  # plot( seq(1,length(L.n.plot),1), L.n.plot, type = 'o', col = "blue", main=c("Screening"))
  # if (ind != 0){abline(v=  ind  );}
  
  return(list(pts = pts.full[[ind]], phi.local = phi.local.1, L.n.plot = L.n.plot, L.n.1 = L.n.1, L.n.2 = L.n.2, omega = omega  ))
  #return(list(pts = pts.full[[ind+1]], ic = ic, phi.local = phi.local.1, L.n.plot = L.n.plot, L.n.1 = L.n.1, L.n.2 = L.n.2, omega = omega  ))
}
...
# show distinct color for posotive and negative value
plot.matrix.2 <- function (phi,p,name = NULL) {
  B <- phi
  if (nrow(B) == 1) {
    B <- matrix(B[, 1:ncol(B)], nrow = 1)
  }
...
first.step.cv.new.blocks <- function(method,data.temp, weight = NULL, lambda.1.cv, lambda.2.cv, p, max.iteration = max.iteration, tol = tol, step.size = (1/2)*10^(-4),cv.index, blocks,cpp= TRUE){
  ...
  for (i in 1:kk) {
    ...
    if ( i == 1){
      if(cpp){
        print("####use rcpp !!!")
        test <- var_break_fit_block_cpp(data.temp, lambda.full[i,1],lambda.full[i,2], p, max.iteration, tol = tol, initial_phi = 0.0+matrix(0.0,k,k*p*n.new), blocks, cv.index)
      }else{
        print("####no rcpp !!!")
        test <- var.break.fit.block(method,data.temp, weight = NULL, lambda.full[i,1],lambda.full[i,2], p, max.iteration = max.iteration, tol = tol, initial.phi = NULL, blocks = blocks, cv.index = cv.index)
    ...
    if ( i > 1 ){
      ...
      if(cpp){
        print("####use rcpp !!!")
        test <- var_break_fit_block_cpp(data.temp, lambda.full[i,1],lambda.full[i,2], p, max.iteration, tol = tol, initial_phi = initial.phi, blocks, cv.index)
        
      }else{
        print("####no rcpp !!!")
        test <- var.break.fit.block(method,data.temp, weight = NULL, lambda.full[i,1],lambda.full[i,2], p, max.iteration = max.iteration, tol = tol, initial.phi = initial.phi, blocks = blocks, cv.index = cv.index)
      }
      flag.full[i] <- test$flag;
    }
    phi.hat.full <- test$phi.hat;
    ...
    for(j in 1:length(ll)){
      ...
      for (iii in 1:(n.new-1))
      {
        ...
        # }
        if ( sum((phi.hat[,((iii-1)*k*p+1):(iii*k*p)] )^2 ) > tol*50   ){
          m.hat <- m.hat + 1; brk.points[m.hat] <- blocks[iii+1];
        }
        # if ( sum((phi.hat[,((iii-1)*k*p+1):(iii*k*p)] )^2 ) > 1   ){
        #   m.hat <- m.hat + 1; brk.points[m.hat] <- blocks[iii+1];
        # }
      }
  ...
  return(list(brk.points = brk.points.final[[lll]], cv = cv, cv1.final = lambda.full[lll,1], cv2.final = lambda.full[lll,2], cv.loc = lll, phi.hat = phi.final[[lll]]))
}
...
detection.check.new <- function(pts.final.block,an.final, brk, N){
  m <- length(brk); len <- rep(0,N);
  for(i in 1:N){len[i] <- length(pts.final.block[[i]]);}
  freq <- as.matrix(table(len)/N);
  pts.final.full.1 <- vector("list",N);
  for(i in 1:N){
    if ( length(pts.final.block[[i]]) > (m-1)   ){pts.final.block[[i]] <- remove.extra.pts(pts.final.block[[i]], brk);print("extra points exists!"); print(i);}
    #if ( length(pts.final.block[[i]]) == (m-1)   ){pts.final.full[[i]] <- pts.final.block[[i]];}
    if ( length(pts.final.block[[i]]) == 0 ) {  pts.final.block[[i]] <- rep(0,m-1); pts.final.full.1[[i]] <- rep(0,m-1);  }
    if ( length(pts.final.block[[i]]) > 0 && length(pts.final.block[[i]]) <= (m-1) ){
      ll <- length(pts.final.block[[i]]); pts.final.full.1[[i]] <- rep(0,m-1);
      for(j in 1:ll){
        if (  pts.final.block[[i]][j] < (brk[1] + (1/5)*(brk[2] - brk[1]) )   ) {pts.final.full.1[[i]][1] <- pts.final.block[[i]][j];}
        for(kk in 2:(m-1)){
          if (  pts.final.block[[i]][j] >=  (brk[(kk)] - (1/5)*(brk[kk] - brk[(kk-1)]) ) && pts.final.block[[i]][j] <  (brk[(kk)] + (1/5)*(brk[kk+1] - brk[(kk)]) )   ){
            pts.final.full.1[[i]][kk] <- pts.final.block[[i]][j];
          }
        }
      }
    }
  ...
  for(i in 1:N){
    if ( length(pts.final.block[[i]]) > 0 && length(pts.final.block[[i]]) <= (m-1) ){
      ll <- length(pts.final.block[[i]]); pts.final.full.2[[i]] <- rep(0,m-1);
      for(j in 1:ll){
        if (  pts.final.block[[i]][j] < (brk[1] + an.final[[i]])   ) {pts.final.full.2[[i]][1] <- pts.final.block[[i]][j];}
        for(kk in 2:(m-1)){
          if (  pts.final.block[[i]][j] >=  (brk[(kk)] - an.final[[i]] ) && pts.final.block[[i]][j] <  (brk[(kk)] + an.final[[i]] )   ){
            pts.final.full.2[[i]][kk] <- pts.final.block[[i]][j];
          }
        }
      }
    }
  ...
  detection <- matrix(0,m,8)
  
  detection[1,1] <- c("break points"); detection[1,2] <- c("truth"); 
  detection[1,3] <- c("mean(errors)"); detection[1,4] <- c("std(errors)"); 
  detection[1,5] <- c("selection rate (1/4)"); detection[1,6] <- c("selection rate new");
  detection[1,7] <- c("mean(location)"); detection[1,8] <- c("std(location)");
  for(i in 1:(m-1)){
    detection[(i+1),1] <- c(i); detection[(i+1),2] <- c(brk[i]/T); 
    loc <- rep(0,N); loc.1 <- rep(0,N);loc.2 <- rep(0,N);
    for(j in 1:N){
      temp <- pts.final.block[[j]]; l <- length(temp); loc[j] <- temp[i];
      temp.1 <- pts.final.full.1[[j]]; l.1 <- length(temp); loc.1[j] <- temp.1[i];
      temp.2 <- pts.final.full.2[[j]]; l.2 <- length(temp.2); loc.2[j] <- temp.2[i];
    }
    loc <-  loc[which(loc!=0)]
    loc.1 <- loc.1[which(loc.1!=0)]; T.new.1 <- length(loc.1); 
    loc.2 <- loc.2[which(loc.2!=0)]; T.new.2 <- length(loc.2); 
    detection[(i+1),3] <- mean(abs(loc-brk[i])/T); detection[(i+1),4] <- sd(abs(loc-brk[i])/T);
    detection[(i+1),5] <- T.new.1/N; detection[(i+1),6] <- T.new.2/N;
    detection[(i+1),7] <- mean(loc.1/T); detection[(i+1),8] <- sd(loc.1/T);
  }
...
detection.check.new.2 <- function(pts.final.block, brk, N){
  m <- length(brk); len <- rep(0,N);
  for(i in 1:N){len[i] <- length(pts.final.block[[i]]);}
  freq <- as.matrix(table(len)/N);
  pts.final.full.1 <- vector("list",N);
...
third.step.final.exhaust <- function(data, p, max.iteration = 1000, tol = tol, step.size = 10^(-3), pts.list, an.2,  eta ){
  N <- length(data[,1]); k <- length(data[1,]);
  n <- length(pts.list);
  final.pts <- rep(0,n);
  ...
  for(i in 1:n){
    ...
    m <- length(pts.temp); 
    if( m <= 1  ) {
      #pts.list.full[[i+1]] <- seq(pts.temp - an, pts.temp +  an, 1 )
      final.pts[i] <- pts.temp;
      print(final.pts[i])
    }
    if( m > 1  ){
      bounds.1 <- vector("list",2*m);
      lb <- max(pts.list.full[[i+1]]) + an.2;
      ub <- min(pts.list.full[[i+1]]) - an.2;
      
      
      for(ii in 1:m){
        bounds.1[[(2*ii-1)]] <- c(lb , pts.temp[ii] - 1 );
        bounds.1[[(2*ii)]] <- c(pts.temp[ii], ub );
      ...
      #   temp
      # }
      # L.n <- as.vector(sapply(c(1:(2*m)), function(jj) r.1[[jj]][[1]]));
      # phi.local <- lapply(c(1:(2*m)), function(jj) r.1[[jj]][[2]]);
      # print(L.n)
      
      
      print("rcpp in step 3")
      L.n <- c()
      phi.local.1 <- c()
      # print(bounds.1)
      for(mm in 1:(2*m)){
        ...
        phi.local <- c(phi.local, try$phi.hat)
      }
      print(L.n)
...
bss.block.select <- function(method, data, weight = NULL, lambda.1.cv, p, max.iteration = max.iteration, tol = tol, step.size = (1/2)*10^(-4), al){
  ...
  while(mm < NN){
    ...
    if ( length(temp$pts.3) > 0  ){pts.final[[mm]] <- temp$pts.3;}
    # if ( length(temp$pts.3) == 0  ){pts.final[[mm]] <- c();}
    
    if ( mm > 1 && length(pts.final[[mm]]) == length(pts.final[[mm-1]])    ){
      ind <- mm;
      print("mmmm"); print(mm);
      # break;
    }
  ...
  return(list( block.size = block.full[[ind]][2], pts.1 = pts.final, pts.3 = pts.final[[ind]], len = len, block.full = block.full ) ) 
}



bss <- function(method, data, lambda.1.cv, lambda.2.cv, p, max.iteration = max.iteration, tol = tol, step.size = (1/2)*10^(-4), blocks, cpp=TRUE){
  ...
  pt1 <- proc.time()
  temp.first <- first.step.cv.new.blocks(method, data, weight = NULL, lambda.1.cv, lambda.2.cv, p, max.iteration = max.iteration, tol = tol, step.size = (1/2)*10^(-4), cv.index, blocks=blocks, cpp)

  # mspe.plot(method,temp.first$cv, c(1:kk), temp.first$cv.final, "1")
  # abline(v = seq(length(lambda.1.cv), length(lambda.1.cv)*(length(lambda.2.cv)-1), length.out =length(lambda.2.cv)-1)+0.5)
  # abline(v= temp.first$cv.loc, col="red")
  
  first.brk.points <- temp.first$brk.points;
  # first.brk.points <- first.brk.points[first.brk.points>8000];
  print(first.brk.points);
  print(paste("first step time:", (proc.time() - pt1)[3]))

  ################## OMEGA LOCAL (a_n in the paper )#######################################
  ########fixed a_n value#######################################
  n <- T - p;
  #an <- floor(2*mean(blocks.size))+floor(( log( (T^0.5)/(mean(blocks.size)) + 2)  )*(((log(n))^2)*(log(k))^2))+1;# the penalty term in the information criterion.
  #an <- floor( mean(blocks.size))+floor((((log(n))^1.5)*(log(k))^1.5))+1;# the penalty term in the information criterion.
  #an <- floor(1.5* mean(blocks.size))+floor((((log(n))^1.5)*(log(k))^1.5))+1;# the penalty term in the information criterion.
  # an <- floor(1.5* mean(blocks.size))+floor((((log(n))^2)*(log(k))^2))+1;# the penalty term in the information criterion.
  #an <- min(an,0.95*(min(first.brk.points)-1-p),0.95*(n - max(first.brk.points)-1));
  #an <- floor(an);
  
  
  ########grid search a_n value#######################################
  # try this for T = 20000
  # try this for T = 5000
  #an.lb <- floor(1.5* mean(blocks.size)) + floor(    (log(n)*log(k))^1.5   ) + 1;# the penalty term in the information criterion.

  # try this for T = 1000, 2000 
  # an.lb <- floor(mean(blocks.size)) + floor( (log(n)*log(k)) ) + 1;# the penalty term in the information criterion.
  # an.lb <- max(floor(3*mean(blocks.size)),floor( (log(n)*log(k))^1 ));# the penalty term in the information criterion.
  
  # an.ub <-  min(0.95*(min(first.brk.points)-1-p),0.95*(n - max(first.brk.points)-1))
  
  # an.ub <-  min(floor(sqrt(n)),floor(mean(blocks.size))*floor( (log(n)*log(k)) ),0.95*(min(first.brk.points)-1-p),0.95*(n - max(first.brk.points)-1))
  # an.ub <-  min(floor(mean(blocks.size))*floor( (log(n)*log(k)) ),0.95*(min(first.brk.points)-1-p),0.95*(n - max(first.brk.points)-1))
  # an.ub <-  min(10*an.lb, 0.95*(min(first.brk.points)-1-p), 0.95*(n - max(first.brk.points)-1))
  # an.ub <-  min(0.95*(min(first.brk.points)-1-p),0.95*(n - max(first.brk.points)-1))
  
  # try this for T = 1000, 2000, 5000 
  # an.grid <- seq(an.lb,  an.ub, length.out = 5);
  # an.grid <- c(6500)      ### 6500 is for using all data
  # an.grid <- c(4000)
  an.grid <- c(3000)
  ...
  print("a_n grid:");
  print(an.grid)
  ############ new omega local for fMRI data##################
  # omega.local <- floor(min(20,0.95*(min(fisrt.brk.points)-1-p),0.95*(n - max(fisrt.brk.points)-1)));
  ##############################
  
  #BIC.res <- rep(0,length(an.grid))
  
  
  SSE.res <- rep(0,length(an.grid))
  ...
  while(an.idx < length(an.grid)  ){
    ...
    if(length(first.brk.points) != 0){
      for(i in 1:length(first.brk.points)){
        if ( first.brk.points[i] < (an-1-p)   ){remove.ind <- c(remove.ind,i);}
        if ( (T-first.brk.points[i]) < (an-1-p)   ){remove.ind <- c(remove.ind,i);}
      }
    }
    ...
    if( length(first.brk.points) != 0){
      ...
      ###################################################################### 
      # n.local <- 2*(length(first.brk.points))*an;
      
      #OMEGA is now selected by data-driven method

      eta <- (1/2)*(log(2*an)*log(k))/(2*an) # the second tuning parameter. This default number seems to be working for many simulation and real data examples!
      # eta <- 0
      pt2 <- proc.time()
      temp <- second.step.local.new(data, eta = eta, p, max.iteration = 1000, tol = tol, step.size = 10^(-3), first.brk.points, an)
      print(paste("Second step time:", (proc.time() - pt2)[3]))
      
      #brk.points after local screening
      ...
      # an.2 <- 4000      ### this an.2 is for using all data
      # an.2 <- 1500
      an.2 <- 3000
      print("an.2:"); print(an.2)
      pt3 <- proc.time()
      while( min(abs(diff(pts.final))) <  2*an.2  ){
        if( length(pts.final) != 0){
          pts.list <- block.finder(pts.final,2*an.2)
          print(pts.list);
          # Use exhaustive search for this step
          pts.final <- third.step.final.exhaust(data, p, max.iteration = 1000, tol = tol, step.size = 10^(-3), pts.list, an.2 , eta )
        }
      }
      print("pts.final:")
      print(pts.final)
      print(paste("Third step time:", (proc.time() - pt3)[3]))
      
      final.pts.res[[an.idx]] <- pts.final
      
      # if(an.idx > 1){
      #   if(length(final.pts.res[[an.idx]]) == length(final.pts.res[[an.idx-1]] ) ){
      #     flag <- c("FALSE");
      #     an.idx.final <- an.idx;
      #     break;
      #   }
  ...
  # for(an.idx in c(1: length(an.grid) )  ){
  #   an <- an.grid[an.idx]
  #   remove.ind <- c();
  #   if(length(first.brk.points) != 0){
  #     for(i in 1:length(first.brk.points)){
  ...
  #     
  #     #OMEGA is now selected by data-driven method
  #     eta <- (1/1)*(log(2*an)*log(k))/(2*an) # the second tuning parameter. This default number seems to be working for many simulation and real data examples!
  #     temp <- second.step.local.new(data, eta = eta, p, max.iteration = 1000, tol = tol, step.size = 10^(-3), first.brk.points, an)
  #     
  #     
  #     #brk.points after local screening
  ...
  #     ###################################################################### 
  #     pts.final <- final.brk.points;
  #     while( min(abs(diff(pts.final))) <  2*an  ){
  #       if( length(pts.final) != 0){
  #         pts.list <- block.finder(pts.final,2*an)
  #         print(pts.list);
  #         # Use exhaustive search for this step
  #         pts.final <- third.step.final.exhaust(data, p, max.iteration = 1000, tol = tol, step.size = 10^(-3), pts.list, an , eta )
  #       }
  #     }
  #     print("pts.final:")
  #     print(pts.final)
  ...
  #   }
  #   an.idx.final <- which.max(loc.final)
  #   an.sel <- an.grid[an.idx.final];
  # }
  
  ####find the length that happen the most
  ## if there are multiple lengths, find the longest one
  if(flag == FALSE){
    loc.final <- rep(0,length(an.grid));
    for(i in 1:length(an.grid)){
      loc.final[i] <- length(final.pts.res[[i]]);
 ...
 return(list(pts.1 = first.brk.points, pts.2 = final.brk.points, pts.3 = final.pts.res[an.idx.final], pts.3.grid = final.pts.res, flag = flag, an = an.sel)) 
}

bss.eeg <- function(method, data, lambda.1.cv, lambda.2.cv, p, max.iteration = max.iteration, tol = tol, step.size = (1/2)*10^(-4), blocks, cpp=TRUE){
  ...
  first.brk.points <- temp.first$brk.points;
  # first.brk.points <- first.brk.points[first.brk.points>2000];
  print(first.brk.points);
  ...
  # try this for T = 1000, 2000 
  # an.lb <- floor(mean(blocks.size)) + floor( (log(n)*log(k)) ) + 1;# the penalty term in the information criterion.
  an.lb <- max(floor(mean(blocks.size)),floor( (log(n)*log(k))^1 ));# the penalty term in the information criterion.
...
bss.full <- function(method, data, lambda.1.cv, lambda.2.cv, p, max.iteration = max.iteration, tol = tol, step.size = (1/2)*10^(-4), blocks, cpp=TRUE){
  ...
  # try this for T = 1000, 2000
  # an.lb <- floor(mean(blocks.size)) + floor( (log(n)*log(k)) ) + 1;# the penalty term in the information criterion.
  #an.lb <- max(floor(mean(blocks.size)),floor( (log(n)*log(k)) ));# the penalty term in the information criterion.
  ...
  for(an.idx in c(1: length(an.grid) )  ){
    ...
    #################################calcuate BIC or MSE for each segment
    n <- length(pts.final);
    pts.list.full <- pts.final
    pts.list.full <- c(1, pts.list.full , T)

    sse.full <- 0
    pts.temp <- pts.list.full[2]
    lb <- pts.list.full[[1]];
    bounds.1 <- vector("list",1);
    bounds.1[[1]] <- c(lb , pts.temp - 1 );
    print("bounds.1:");print(bounds.1)

    L.n <- rep(0,2);
    r.1 <- foreach(mm=1, .inorder = FALSE, .export = c("var.lasso.brk","soft","pred")) %dopar% {
      temp <- vector("list",2);
      data.temp <- data[(bounds.1[[mm]][1]):(bounds.1[[mm]][2]),];
      try <- var.lasso.brk(data = data.temp, weight = NULL, eta, p, max.iteration = 1000, tol = tol)
      temp[[1]] <- try$pred.error;
    ...
    L.n <- as.vector(sapply(c(1), function(jj) r.1[[jj]][[1]]));
    print(L.n[1])
    sse.full <-  sse.full +  abs(L.n[1])
    ...
    for( i in 1:n){
      ...
      bounds.1 <- vector("list",1);
      #lb <- pts.list.full[[i]];
      ub <- pts.list.full[[i+2]] ;
      #bounds.1[[1]] <- c(lb , pts.temp - 1 );
      bounds.1[[1]] <- c(pts.temp, ub - 1  );
      print("bounds.1:");print(bounds.1)
  ...
  return(list(pts.1 = first.brk.points, pts.2 = final.brk.points, pts.3 = final.pts.res[which.min(SSE.res)], pts.3.grid = final.pts.res, SSE.res = SSE.res))
}
