...
\begin{table}[!ht]
    ...
    \hline
        ...
        FVARSeg + kmeans  & (3, 0.125, 0.500, 0.800) & 0.12 & (0.0182, --,~~~--)~~~~~~~ & (0.0118, --,~~~--)~~~~~~~ & 0.086 \\
    \hline\hline
    \end{tabular}}
\end{table}

From the results we can see that our iterative clustering algorithm outperforms each baseline on every metric. Compared to SBS+kmeans, the per-cluster mean estimates are closer to the truth, the standard deviation is roughly an order of magnitude smaller, and the global clustering accuracy is $1.000$ versus $0.843$. BSS+kmeans tends to under-estimate the number of groups ($\widehat{g} \approx 2.72$) and shows a sizable bias on the leftmost change point (0.215 vs the truth 0.125), with accuracy $0.815$. The factor-adjusted FVARSeg method targets factor-driven VAR processes; on the purely sparse VAR data used in G4, it returns no change points in roughly $88\%$ of the replications ($\widehat{g} \approx 0.12$), so neither its mean estimates of the second and third change points nor its global accuracy are informative. All four methods are evaluated on the same G4 setting (50 replications for Ours, SBS, BSS; 48 replications for FVARSeg), so the rows are directly comparable.
